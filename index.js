...
// Owner numbers
const ownerNumber = ["923207047366"];
...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("SPIDER-MD~")) {
    console.log("❌ Invalid SPIDER-MD session format");
    process.exit(1);
  }
  ...
  console.log("♻️ SPIDER-MD session restored successfully");
}
    ...
    res.send(`
      ...
      <body>
        <div class="container">
          ...
          <p>Owner: ${config.OWNER_NAME || 'REHAN'}</p>
        </div>
      </body>
      </html>
    `);
...
async function getProfilePicture(sock, jid) {
  try {
    ...
    return 'https://up6.cc/2026/05/17801647525761.png'; // Default image
  }
}
...
async function connectToWA() {
  ...
  console.log(`🤖 SPIDER-MD Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ SPIDER-MD Bot connected to WhatsApp!");
      ...
      const aliveMsg = `*╭──────────────●●►*\n> *SPIDER-MD CONNECTED SUCCESSFULLY*\n\n> *Type ${prefix}menu to view commands*  \n\n*╭⊱✫ SPIDER MD ✫⊱╮*\n*│✫📂 Bot Name: ${botConfig.BOT_NAME}*\n*│✫🛡️ Owner: ${config.OWNER_NAME}*\n*│✫♻️ Prefix: ${prefix}*\n*│✫🌍 Mode: ${config.MODE}*\n*│✫⏰ Uptime: ${runtime(process.uptime())}*\n*╰──────────────●●►*\n\n> Enjoy Using SPIDER MD`;
      
      // Image URL for connection message
      const imageUrl = 'https://up6.cc/2026/05/17801647525761.png';
      
      try {
        // Send to owner with image
        sock.sendMessage(ownerNumber[0] + '923207047366@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
          // Fallback to text if image fails
          sock.sendMessage(ownerNumber[0] + '923207047366@s.whatsapp.net', { text: aliveMsg });
        });
      ...
      for (const participant of participants) {
        ...
        if (action === 'add') {
          ...
          if (settings.goodbye) {
            try {
              // Get user's profile picture
              const ppUrl = await getProfilePicture(sock, participant).catch(() => 'https://up6.cc/2026/05/17801647525761.png');
